...
#
# Description - Creating Urban-Rural Classificiation Lookup Files
...
UR_Lookup <- read_csv(glue("{data_filepath}/DZ2011_SGUR2016_Lookup.csv")) %>% 
  select(-DZ_CODE) %>% 
  distinct() %>% 
  ...
  select(UR2_2016, UR2_2016_name, UR3_2016, UR3_2016_name, UR6_2016, 
         UR6_2016_name, UR8_2016, UR8_2016_name)
...
DZ2011_SGUR2016_Lookup <- read_csv(glue("{data_filepath}/", 
                                        "DZ2011_SGUR2016_Lookup.csv")) %>% 
  ...
  left_join(UR_Lookup) %>% 
  select(DataZone2011, UR2_2016, UR2_2016_name, UR3_2016, UR3_2016_name, 
         UR6_2016, UR6_2016_name, UR8_2016, UR8_2016_name)

saveRDS(DZ2011_SGUR2016_Lookup, 
        glue("{lookups_filepath}/DataZone2011_urban_rural_2016.rds"))
...
OA2011_SGUR2016_Lookup <- read_csv(glue("{data_filepath}/", 
                                        "OA2011_SGUR2016_Lookup.csv")) %>% 
  ...
  arrange(OA2011) %>% 
  left_join(UR_Lookup) %>% 
  select(OA2011, UR2_2016, UR2_2016_name, UR3_2016, UR3_2016_name, UR6_2016, 
         UR6_2016_name, UR8_2016, UR8_2016_name)

saveRDS(OA2011_SGUR2016_Lookup, 
        glue("{lookups_filepath}/OA2011_urban_rural_2016.rds"))
...
SETT2016_SGUR2016_Lookup <- read_csv(glue("{data_filepath}/", 
                                          "SETT2016_SGUR2016_Lookup.csv")) %>% 
  ...
  arrange(Settlement) %>% 
  left_join(UR_Lookup) %>% 
  select(Settlement, Settlement_Name, Population, SettPopClass, UR2_2016, 
         UR2_2016_name, UR3_2016, UR3_2016_name, UR6_2016, UR6_2016_name, 
         UR8_2016, UR8_2016_name)

saveRDS(SETT2016_SGUR2016_Lookup, 
        glue("{lookups_filepath}/Settlement_urban_rural_2016.rds"))
...
# Rename variables and sort by pc8 ascending, Date_of_Introduction descending 
# and Date_of_Deletion ascending.

PC2017_2_SGUR2016_Lookup <- read_csv(glue("{data_filepath}/", 
                                          "PC2017_2_SGUR2016_Lookup.csv")) %>% 
  rename(pc8 = POSTCODE, 
  ...
  left_join(UR_Lookup) %>% 
  select(pc8, SplitChar, Date_of_Introduction, Date_of_Deletion, UR2_2016, 
         UR2_2016_name, UR3_2016, UR3_2016_name, UR6_2016, UR6_2016_name, 
         UR8_2016, UR8_2016_name)
...
PC2017_2_SGUR2016_Lookup %<>% filter(duplicate == 0)
...
PC2017_2_SGUR2016_Lookup %<>%  
  ...
  select(pc7, pc8, SplitChar, Date_of_Introduction, Date_of_Deletion, UR2_2016, 
         UR2_2016_name, UR3_2016, UR3_2016_name, UR6_2016, UR6_2016_name, 
         UR8_2016, UR8_2016_name)

saveRDS(PC2017_2_SGUR2016_Lookup, 
        glue("{lookups_filepath}/Postcode_urban_rural_2016.rds"))
