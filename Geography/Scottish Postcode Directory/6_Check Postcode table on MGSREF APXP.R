##########################################################
# Check Postcode table on MGSREF APXP
# Calum Purdie
...
# Code for checking the postcode table on MGSREF APXP
##########################################################
...
APXP_connection <- odbc::dbConnect(
  drv = odbc::odbc(),
  dsn = "APXP",
  uid = rstudioapi::askForPassword("APXP Username:"),
  pwd = rstudioapi::askForPassword("APXP Password:"))
...
postcode <- tbl(APXP_connection, "POSTCODE") %>%
  collect() %>% 
  arrange(POSTCODE)
...
pc_geo_vw <- tbl(APXP_connection, "PC_GEOGRAPHY_VW") %>%
  collect() %>% 
