...
data_filepath <- glue("{base_filepath}/2_Population/Small Area Population estimates/Source Data")
lookup_filepath <- glue("{base_filepath}/2_Population/Small Area Population estimates/Lookup Files/R Files")
lookup_filepath_SPSS <- glue("{base_filepath}/2_Population/Small Area Population estimates/Lookup Files") #2021 ADDITION FILE PATH FOR WRITING OUT SPSS
simd2020v2_filepath <- glue("{base_filepath}/3_Deprivation/SIMD/Lookup Files/SIMD 2020")
simd2016_filepath <- glue("{base_filepath}/3_Deprivation/SIMD/Lookup Files/SIMD 2016/R Files")
...
read_data <- function(pop_data, pop_year, pop_gender){
  #IMPORT CELL RANGE WILL VARY YEAR ON YEAR DUE TO INSERTAION OR NOT OF BLANK ROWS IN EXCEL INCOMING FROM nrs CHECK RANGE AND IF BLANK COLUMNS NEED SKIPPING OVER (...5)
  data <- read_excel(glue("{data_filepath}/{pop_data}.xlsx"), 
                     sheet = 1, range = "A5:CQ6981") %>% 
    #mutate(Year = pop_year) %>%
    #mutate(Sex = pop_gender) %>% 
    select(-c(DataZone2011Name, CouncilArea2019Name)) %>% 
...
write_sav(dz2011_pop_est,glue("{lookup_filepath_SPSS}/{new_dz_estimates}.sav"))
...
write_sav(dz2011_pop_est_5y,glue("{lookup_filepath_SPSS}/{new_dz_estimates_5y}.sav"))
...
write_sav(iz2011_pop_est,glue("{lookup_filepath_SPSS}/{new_iz_estimates}.sav"))
...
write_sav(iz2011_pop_est_5y,glue("{lookup_filepath_SPSS}/{new_iz_estimates_5y}.sav"))
