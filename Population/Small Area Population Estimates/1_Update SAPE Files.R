...
#
# Description - This document is for updating small area population estimate files for the most recent release. 
#               This file should be updated yearly for each subsequent release.
#
...
# Read in female data
f_2018 <- read_excel(file.path(data_filepath, "Datazone2011_2018_f.xlsx"), sheet = 1, range = "A6:CR6982") %>% 
  mutate(Year = 2018) %>%
  mutate(Sex = 'F')

# Read in male data
m_2018 <- read_excel(file.path(data_filepath, "Datazone2011_2018_m.xlsx"), sheet = 1, range = "A6:CR6982") %>% 
  mutate(Year = 2018) %>%
  mutate(Sex = 'M')
...
# Remove unnecessary columns - CouncilArea2011Name and X__2
mandf <- lapply(mandf, function(x) x[,-c(3,5)])

# Rename Columns
# Function changes each column name to the respective name listed in the function, in order
...
DZ2011_simd2016 <- readRDS(file.path(lookups_filepath, "Deprivation", "DataZone2011_simd2016.rds"))

# Match to DataZone2011_SIMD2016 and HSCPLocality to get other geography level and SIMD information
# Remove pop_2014 and rearrange variables
DZ2011_2018 <- mandf_2018 %>%
  left_join(DZ2011_simd2016, by = "DataZone2011") %>% 
  select(-pop_2014) %>% 
  select(Year, DataZone2011, DataZone2011Name, Sex, age0:age90plus, total_pop, everything())

### 3.2 - Add to DataZone2011_pop_est_2011_2017 File ----

# Read in DataZone2011_pop_est_2011_2017 file
DZ2011_pop_est_2011_2017 <- readRDS(file.path(base_filepath, "Lookup Files", "R Files", "Archive", "DataZone2011_pop_est_2011_2017.rds"))

### THIS IS ONLY REQUIRED ONCE ###
# Need to add in the updated 2019 geographies in the DZ2011_2018 and DataZone2011_pop_est_2011_2017 files
...
# Update DZ2011_2018
DZ2011_2018 <- DZ2011_2018 %>% 
  left_join(geo_names)

# Update DZ2011_pop_est_2011_2017
DZ2011_pop_est_2011_2017 <- DZ2011_pop_est_2011_2017 %>%
  left_join(geo_names) %>%
...
DZ2011_pop_est_2011_2018 <- DZ2011_pop_est_2011_2017 %>%
  ...
  select(Year, DataZone2011, DataZone2011Name, Sex, age0:IntZone2011, IntZone2011Name, 
         HB2019, HB2019Name, HB2018, HB2014, HSCP2019, HSCP2019Name, HSCP2018, HSCP2016,
         CA2019, CA2019Name, CA2018, CA2011, simd2016rank:simd2016_crime_rank) %>% 
  arrange(Year, DataZone2011, desc(Sex))

# Save as .RDS file
saveRDS(DZ2011_pop_est_2011_2018, file.path(base_filepath, "Lookup Files", "R Files", "DataZone2011_pop_est_2011_2018.rds"))



### 3.3 - Compute Age Groups and Add to DataZone2011_pop_est_2011_2016_5year_agegroups File ----

# Read in DataZone2011_pop_est_2011_2017_5year_agegroups file
DZ2011_pop_est_2011_2017_5year_agegroups <- readRDS(file.path(base_filepath, "Lookup Files", "R Files", "Archive", "DataZone2011_pop_est_5year_agegroups_2011_2017.rds"))

# Need to add in the updated 2019 geographies in the DZ2011_pop_est_2011_2017_5year_agegroups file
# Do this using the geo_names lookup file
DZ2011_pop_est_2011_2017_5year_agegroups <- DZ2011_pop_est_2011_2017_5year_agegroups %>%
  left_join(geo_names) %>%
  arrange(Year, DataZone2011, desc(Sex))
...
DZ2011_pop_est_2011_2018_5year_agegroups <- DZ2011_2018 %>%
  ...
  full_join(DZ2011_pop_est_2011_2017_5year_agegroups) %>% 
  select(Year, DataZone2011, DataZone2011Name, Sex, ageg04:ageg8589, ageg90plus, 
         total_pop, IntZone2011, IntZone2011Name, HB2019, HB2019Name, HB2018, HB2014, 
         HSCP2019, HSCP2019Name, HSCP2018, HSCP2016, CA2019, CA2019Name, CA2018, CA2011, everything()) %>%
  arrange(Year, DataZone2011, desc(Sex))

# Save as .RDS file
saveRDS(DZ2011_pop_est_2011_2018_5year_agegroups, file.path(base_filepath, "Lookup Files", "R Files", "DataZone2011_pop_est_5year_agegroups_2011_2018.rds"))
...
iz_names <- read.csv(file = "https://www.opendata.nhs.scot/dataset/9f942fdb-e59e-44f5-b534-d6e17229cc7b/resource/e3e885cc-2530-4b3c-bead-9eda9782264f/download/geography_codes_and_labels_iz2011_19082019.csv") %>% 
  select(IZ2011, IZ2011Name) %>% 
  mutate_if(is.factor, as.character) %>% 
  rename(IntZone2011 = IZ2011, IntZone2011Name = IZ2011Name)

IZ2011_pop_est_2011_2018 <- IZ2011_pop_est_2011_2018 %>% 
  left_join(iz_names) %>% 
  select(Year, IntZone2011, IntZone2011Name, Sex, age0:total_pop)

# Save file as .RDS
saveRDS(IZ2011_pop_est_2011_2018, file.path(base_filepath, "Lookup Files", "R Files", "IntZone2011_pop_est_2011_2018.rds"))
...
# Sort data by Year, IntZone2011 and Sex
IZ2011_pop_est_2011_2018_5year_agegroups <- DZ2011_pop_est_2011_2018_5year_agegroups %>%
  group_by(Year, IntZone2011, Sex) %>%
...
IZ2011_pop_est_2011_2018_5year_agegroups <- IZ2011_pop_est_2011_2018_5year_agegroups %>% 
  left_join(iz_names) %>% 
  select(Year, IntZone2011, IntZone2011Name, Sex, ageg04:total_pop)

# Save file as .RDS
saveRDS(IZ2011_pop_est_2011_2018_5year_agegroups, file.path(base_filepath, "Lookup Files", "R Files", "IntZone2011_pop_est_5year_agegroups_2011_2018.rds"))
...
data_check(DZ2011_pop_est_2011_2018_5year_agegroups, "DataZone2011")
...
data_check(IZ2011_pop_est_2011_2018_5year_agegroups, "IntZone2011")
...
DZ_5y_total <- DZ2011_pop_est_2011_2018_5year_agegroups %>%
  group_by(Year) %>%
...
IZ_5y_total <- IZ2011_pop_est_2011_2018_5year_agegroups %>%
  group_by(Year) %>%
...
DZ2011_lower <- readRDS(file.path(base_filepath, "Lookup Files", "R Files", "DataZone2011_pop_est_2011_2018.rds")) %>%
  clean_names() %>%
...
saveRDS(DZ2011_lower, file.path(base_filepath, "Lookup Files", "R Files", "DataZone2011_pop_est_2011_2018.rds"))


# Convert DataZone2011_5y

DZ2011_5y_lower <- readRDS(file.path(base_filepath, "Lookup Files", "R Files", "DataZone2011_pop_est_5year_agegroups_2011_2018.rds")) %>%
  clean_names() %>%
  rename(datazone2011 = data_zone2011, datazone2011name = data_zone2011name, 
         intzone2011 = int_zone2011, intzone2011name = int_zone2011name)

saveRDS(DZ2011_5y_lower, file.path(base_filepath, "Lookup Files", "R Files", "DataZone2011_pop_est_5year_agegroups_2011_2018.rds"))

# Convert IntZone2011

IZZ2011_lower <- readRDS(file.path(base_filepath, "Lookup Files", "R Files", "IntZone2011_pop_est_2011_2018.rds")) %>%
  clean_names() %>%
  rename(intzone2011 = int_zone2011, intzone2011name = int_zone2011name)

saveRDS(IZZ2011_lower, file.path(base_filepath, "Lookup Files", "R Files", "IntZone2011_pop_est_2011_2018.rds"))


# Convert IntZone2011_5y

IZ2011_5y_lower <- readRDS(file.path(base_filepath, "Lookup Files", "R Files", "IntZone2011_pop_est_5year_agegroups_2011_2018.rds")) %>%
  clean_names() %>%
  rename(intzone2011 = int_zone2011, intzone2011name = int_zone2011name)

saveRDS(IZ2011_5y_lower, file.path(base_filepath, "Lookup Files", "R Files", "IntZone2011_pop_est_5year_agegroups_2011_2018.rds"))
...
DZ2011_pop_est_2011_2018 <- readRDS(file.path(base_filepath, "Lookup Files", "R Files", "DataZone2011_pop_est_2011_2018.rds")) %>% 
...
DZ2011_pop_est_2001_2010 <- readRDS(file.path(base_filepath, "Lookup Files", "R Files", "DataZone2011_pop_est_2001_2010.rds")) %>% 
...
write_csv(DZ2011_pop_est_OD, file.path(open_data_filepath, "DZ2011-pop-est_23082019.csv"))
...
IZ2011_pop_est_2011_2018 <- readRDS(file.path(base_filepath, "Lookup Files", "R Files", "IntZone2011_pop_est_2011_2018.rds")) %>% 
...
IZ2011_pop_est_2001_2010 <- readRDS(file.path(base_filepath, "Lookup Files", "R Files", "IntZone2011_pop_est_2001_2010.rds")) %>% 
